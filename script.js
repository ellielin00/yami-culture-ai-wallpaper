const works = [
  {
    value: "See Through the Customer’s Eyes · 透过客户视角看问题",
    en: "Clearing the Way with Customers in Mind",
    cn: "《循迹清障，看见所需》",
    ...
    prompts: [
      ...
      ["Final direction · 最终落地", "保持完整迷宫、弯道文字和顾客位置。长绒大米位于左下方，推着专业步进式除草机沿迷宫墙前进；刀头正接触绿篱，前方保留明显草层、碎叶和短枝，身后形成已经清理完成的整洁道路。整体色调明亮通透，顾客面部自然清晰。"]
    ]
  },
  {
    value: "Empathy in Everything We Do · 同理心贯穿一切",
    ...
    prompts: [
      ...
      ["Final direction · 最终落地", "保持长绒大米与小米温和对望、指尖相对但不接触。水面涟漪自然汇聚成清晰可辨的 Yami Logo。英文 EMPATHY IN EVERYTHING WE DO 使用系列统一的高窄粗体，位于上方中央，保留安全边距；文字采用暖奶油白，加入轻微纸张颗粒、夕阳透光和极浅阴影，与云层及金色环境光自然融合。"]
    ]
  },
  {
    value: "Relentless Drive · 拼尽全力，永不止步",
    ...
    prompts: [
      ...
      ["Final direction · 最终落地", "短至中长绒大米穿完整红色骑行上衣与深色骑行裤，身体明显压低并向前倾，胸口贴近车把，双臂弯曲发力，神情紧张、专注、奋力。头盔完整覆盖头顶，绒毛被风向后吹动。其他参赛者保持独特毛绒轮廓，尘土与飞石强化一骑绝尘的速度，整体色彩明亮鲜活。"]
    ]
  },
  {
    value: "Think Beyond Limits, Dive Deep to Solve · 思维无界，深度求解",
    ...
    prompts: [
      ...
      ["Final direction · 最终落地", "大米与小米保持准确的长绒毛绒形象和专注神情。最外层右侧的折叠出口透入暖金色阳光，出口外呈现明亮的加州海岸、蓝天、棕榈树、海湾公路与花园建筑。标题 THINK BEYOND LIMITS, DIVE DEEP TO SOLVE 位于左上方，使用砖红棕色高窄粗体，带轻微复古印刷斑驳感，与纸张纹理自然融合。"]
    ]
  },
  {
    value: "Do What’s Right, Own the Outcome · 做正确的事，对结果负责",
    ...
    prompts: [
      ...
      ["Final direction · 最终落地", "大米保持强力踢球姿势，角色尺寸约为普通球员的两倍，红色球服背面清晰印有 Yami Logo；小米同步跟进。足球放大约两倍，在半空沿明确轨迹飞向球门。守门员为与大米小米轮廓差异明显的蓝色长耳毛绒角色，身体完全舒展、表情专注卖力。保持完整球门、球场线和文字可读性。"]
    ]
  },
  {
    value: "Move Fast, Move Together · 高效协作，齐头并进",
...
function render(index, initial=false) {
  ...
  const update = () => {
    ...
    document.querySelector("#title-cn").textContent=w.cn;
    document.querySelector("#desc-en").textContent=w.descEn;
    document.querySelector("#desc-cn").textContent=w.descCn;
    document.querySelector("#prompt-count").textContent=`${w.prompts.length} prompts`;
    document.querySelector("#caption").textContent=`Final iterative production record for “${w.en}”.`;
    document.querySelector("#prompts").innerHTML=w.prompts.map((p,i)=>`<article class="prompt-card"><span>PROMPT ${String(i+1).padStart(2,"0")}</span><h5>${p[0]}</h5><p>${p[1]}</p></article>`).join("");
    tabs.forEach((t,i)=>{t.classList.toggle("active",i===active);t.setAttribute("aria-current",i===active?"true":"false")});
    requestAnimationFrame(()=>visual.classList.remove("changing"));
  };
  initial ? update() : setTimeout(update,180);
}
